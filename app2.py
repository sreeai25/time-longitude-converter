...
# ---------------------------
for key, val in [("active_lon", 0.0), ("clicked_lat", 0.0), ("mode", "Longitude → Time Zone"),
...
with left_col:
    ...
    # Mode selection
    st.radio("Conversion mode", ["Longitude → Time Zone", "Time Zone → Longitude"],
             index=0 if st.session_state.mode.startswith("Longitude") else 1, key="mode")
    ...
    if mode == "Longitude → Time Zone":
        ...
        direction = st.selectbox("Direction", ["E (+)", "W (-)"],
                                 index=0 if dir_default.startswith("E") else 1, key="man_dir")
        ...
        sec = st.number_input("Seconds (0.000–59.999)", min_value=0.0, max_value=59.999,
                              value=round(cur_sec, 6), format="%.6f", key="man_sec")
        ...
        s = st.number_input("Seconds (0.0–59.999)", min_value=0.0, max_value=59.999,
                              value=round(cur_s, 6), format="%.6f", key="man_tz_s")
...
with right_col:
    ...
    slider_val = st.slider("Longitude slider (±180°)", -180.0, 180.0,
                           float(st.session_state.active_lon), step=0.1, key="slider_lon")
    ...
    # Display selected values dynamically from session_state
